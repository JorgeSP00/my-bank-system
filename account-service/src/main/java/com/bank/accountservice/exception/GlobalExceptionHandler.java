...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleAccountAlreadyExists(AccountAlreadyExists ex) {
        String errorId = UUID.randomUUID().toString();
        log.warn("[ErrorId: {}] Account already exists - Message: {}", errorId, ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorResponse(HttpStatus.CONFLICT.value(), ex.getMessage(), errorId));
    }
    ...
    public ResponseEntity<ErrorResponse> handleInvalidAccountData(InvalidAccountData ex) {
        String errorId = UUID.randomUUID().toString();
        log.warn("[ErrorId: {}] Invalid account data - Message: {}", errorId, ex.getMessage());
    ...
    @ExceptionHandler(NotPossibleToUpdateAccount.class)
    public ResponseEntity<ErrorResponse> handleNotPossibleToUpdateAccount(NotPossibleToUpdateAccount ex) {
        String errorId = UUID.randomUUID().toString();
        log.warn("[ErrorId: {}] Not possible to update account - Message: {}", errorId, ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorResponse(HttpStatus.CONFLICT.value(), ex.getMessage(), errorId));
    }
